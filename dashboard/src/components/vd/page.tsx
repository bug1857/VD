    ...
    <div className="min-h-screen bg-canvas">
      ...
      <main className="mx-auto max-w-[1680px] px-8 pb-28 xl:px-12">
        <section className="grid grid-cols-1 gap-10 pt-14 lg:grid-cols-[minmax(0,1fr)_300px] lg:gap-20">
          <div>
            <p className="mono text-[11px] tracking-[0.08em] text-ink-4">{eyebrow.toUpperCase()}</p>
            <h1 className="mt-3 max-w-[26ch] text-[32px] font-medium leading-[1.26] tracking-[-0.028em] text-ink">
              {title}
            </h1>
            <p className="mt-4 max-w-[68ch] text-[17px] leading-[1.6] text-ink-2">{lede}</p>
            {blocked && (
          ...
          <dl className="space-y-3 pt-2 text-[13px]">
            {facts.map((f) => (
              <div key={f.k} className="flex items-baseline gap-4 border-b border-line pb-2.5">
...
export function SectionHead({ title, note }: { title: string; note?: string | undefined }) {
  return (
    <div className="flex items-baseline justify-between gap-8">
      <h2 className="text-[13.5px] font-medium tracking-[-0.01em] text-ink-2">{title}</h2>
      {note && <p className="text-right text-[12.5px] text-ink-4">{note}</p>}
    </div>
  );
}
...
export function KeyRow({
  ...
  return (
    <div className="group flex items-baseline gap-6 border-t border-line py-2.5 transition-colors duration-150 hover:bg-hover/40">
      ...
      {sub && <span className="ml-auto text-right text-[12.5px] text-ink-4">{sub}</span>}
    </div>
  );
}

export function Prose({ children }: { children: ReactNode }) {
  return <p className="max-w-[92ch] text-[13px] leading-[1.68] text-ink-3">{children}</p>;
}
