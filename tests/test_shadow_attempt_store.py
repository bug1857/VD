...
import vdbench.shadow_window as shadow_window
...
class ShadowAttemptLifecycleTests(unittest.TestCase):
    ...
    def test_forked_child_close_cannot_release_parent_lock(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(blocked.stdout.strip(), "SHADOW_ATTEMPT_STORE_BUSY")
            store.close()
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with _store(root / "attempts.sqlite3") as store:
                ...
                def terminalize_without_permit() -> None:
                    ...
                    except Exception as exc:  # test captures exact boundary
                        errors.append(exc)
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with _store(root / "attempts.sqlite3") as store:
                ...
                if pid == 0:
                    ...
                    except Exception as exc:  # child reports exact refusal
                ...
                self.assertEqual(outcome, "SHADOW_ATTEMPT_PERMIT_INVALID")
                store.complete_attempt(
                    identity,
                    permit=permit,
                    envelope=_envelope(
