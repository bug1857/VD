...
from vdbench.canary_admission import Stage4AdmissionResult
...
class CanarySerialRunnerTests(unittest.TestCase):
    ...
    def _admission(self, *, admitted: bool = True, plan_sha256: str | None = None) -> Stage4AdmissionResult:
        return Stage4AdmissionResult(
            admitted=admitted,
            reason_codes=() if admitted else ("RUNTIME_PREFLIGHT_INCOMPLETE",),
            plan_sha256=self.schedule.plan_sha256 if plan_sha256 is None else plan_sha256,
            policy_audit_id="exp009-offline-serial-test",
    ...
    def _runner(
        ...
        *,
        admission: Stage4AdmissionResult | None = None,
    ...
    def test_refused_or_mismatched_admission_dispatches_nothing(self) -> None:
        cases = (
            ...
            self._admission(plan_sha256="0" * 64),
        )
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            for index, admission in enumerate(cases):
                with self.subTest(index=index):
                    ...
                    runner, source, executor = self._runner(ledger, admission=admission)
                    result = runner.run()
