...
| FR-071 | An LKG value SHALL be qualified only from two complete, passing, identity-consistent, globally adjacent 1,200-query qualification epochs at the same eligible `ef`, each composed of six consecutive, disjoint 200-query monitor windows per FR-021. If epoch 1 uses source-window sequence numbers N..N+5, epoch 2 SHALL use N+6..N+11 with no eligible same-lineage window skipped between them. Qualification SHALL use the exact observed mean capped recall and exact nearest-rank observed p95 latency over each epoch's realized observations (no population-level confidence claim), plus health, correctness, and rollback-clean status for every constituent window. | LKG epoch-adjacency/restart tests. |
