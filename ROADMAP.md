...
| Host observation recorder and shadow worker | not started | CRITICAL | ADR-007 / EXP-008 define the framework-neutral foreground recorder, read-only background executor, and reference gateway. No host application exists in this repository; implementation must not retrofit `runner.py` or claim external serving traffic. |
...
EXP-001 supplies the verified Milvus range/threshold baseline. EXP-005 supplies verified L2 and COSINE stationary live-shadow evidence. EXP-006 verifies the standalone workload monitor's offline restart recovery, integrity rejection, bounded processing, and DRY_RUN non-actuation behavior at commit `6650c06`. EXP-007 verifies the durable single-host event-source/outbox at commit `ad635c7`. ADR-007 and EXP-008 now define the remaining Core integration gap: a host-owned non-blocking sampler plus background shadow-audit worker that creates the completed traces the source/outbox already accepts.

Options considered:

- **Implement the ADR-007 reference host recorder and worker offline:** use new modules and dependency-injected fake executor/publisher tests to prove foreground isolation, bounded queue behavior, FIFO/identity-safe grouping, restart loss accounting, and no policy/actuation imports before any live ENV-001 call.
...
**Recommendation:** Implement ADR-007 through a framework-neutral recorder, injected background executor, and reference gateway—not through `runner.py` or a new web service. Prove offline foreground isolation and fail-closed grouping first, then run EXP-008 against ENV-001 in DRY_RUN only. No automatic actuation is authorized.
