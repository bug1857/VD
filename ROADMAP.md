...
**Recommendation:** Do not start a candidate query automatically. First repair and verify the Stage-4 recall/latency evidence-binding boundary: current unintegrated draft work can otherwise combine provenance-unbound evidence and must not be promoted. Then build the remaining read-only qualified-LKG/admission evidence path. An exact human-signed grant remains a separate human-only prerequisite for the frozen transition. Until every gate is satisfied, the correct operational state is DRY_RUN/LKG-only; no safer autonomous substitute can prove controlled live no-interference or create external approval authority.
