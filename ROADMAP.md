...
EXP-001 supplies the verified Milvus range/threshold baseline. EXP-005 supplies verified L2 and COSINE stationary live-shadow evidence. EXP-006 verifies the standalone workload monitor's offline restart recovery, integrity rejection, bounded processing, and DRY_RUN non-actuation behavior at commit `6650c06`. EXP-007 verifies the durable single-host event-source/outbox at commit `ad635c7`. ADR-007's offline recorder/worker/reference-gateway, lazy read-only Milvus executor, real read-only serving adapter, and complete two-stream host→outbox→monitor composition are implemented and under review. The remaining Core integration gap is an EXP-008 runner that builds this reviewed composition from verified ENV-001/DATASET-001 inputs and captures live DRY_RUN evidence.

Options considered:

- **Implement the EXP-008 runner and offline construction tests:** load only verified DATASET-001/ENV-001 configuration and frozen baselines, construct the reference gateway/serving executor/shadow executor/outbox/monitor composition, persist raw evidence and immutable run metadata, and refuse live execution without a successful explicit preflight. No policy actuation is permitted.
...
**Recommendation:** Build the EXP-008 runner next, with fake-client construction and failure tests before contacting ENV-001. It is the final evidence harness needed to validate the whole reference path, still in DRY_RUN only. No automatic actuation is authorized.
