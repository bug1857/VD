...
EXP-001 supplies the verified Milvus range/threshold baseline. EXP-005 supplies verified L2 and COSINE stationary live-shadow evidence. EXP-006 verifies the standalone workload monitor's offline restart recovery, integrity rejection, bounded processing, and DRY_RUN non-actuation behavior at commit `6650c06`. EXP-007 verifies the durable single-host event-source/outbox at commit `ad635c7`. ADR-007's offline recorder/worker/reference-gateway, lazy read-only Milvus executor, real read-only serving adapter, strict monitor-audit sink, and complete two-stream host→outbox→monitor composition are implemented and under review. EXP-008 now has clean-commit stationary live evidence at `2403799` (`artifacts/exp-008/run-20260803T171620Z/`): two complete metric streams reach `NO_DRIFT → NO_CHANGE` with no gRPC-fork warning and no actuation. Its H4 capture/fresh-finalization harness is fake-component tested; the remaining Core integration gap is the pre-registered deliberate live failure-probe evidence, so EXP-008 is not yet verified.

Options considered:

- **Run the commit-pinned EXP-008 deliberate live failure probes:** use the reviewed DATASET-001/ENV-001 configuration and frozen baselines, preserve the completed stationary capture, and capture raw evidence for each registered fail-closed scenario. Independently verify every checksum and no-actuation flag, then decide whether EXP-008 can be marked verified. No policy actuation is permitted.
...
**Recommendation:** Implement and execute the registered deliberate failure-probe harness against the reviewed environment next. Treat the existing stationary evidence as incomplete until each failure scenario and independent artifact review are complete. No automatic actuation is authorized.
