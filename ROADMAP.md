...
EXP-001 supplies the verified Milvus range/threshold baseline. EXP-005 verifies L2 and COSINE stationary live-shadow evidence. EXP-006 verifies offline monitor safety/recovery; EXP-007 verifies the durable single-host trace source/outbox; and ADR-007 / EXP-008 verify the reference host-observation path through live read-only Milvus queries, detector, and `DRY_RUN` policy. EXP-009 Stages 1–3 verify the 60-of-600 finite-manifest workload and calibrated bounds, offline Ed25519 approval/exact routing partition/lifecycle, restart and expiry failback, rollback containment, and a persistent automatic-action interlock. The sealed Stage-4 offline-composition bundle at `artifacts/exp-009/run-20260804T112128Z/`, generated at `2d56463`, verifies the original fake-only schedule/source/ledger/evaluator seam. The separately designed candidate-capable `Stage4LiveRunner` has fake-only evidence at `artifacts/exp-009/run-20260804T141850Z/`, generated from `920aab9` and published at `1614521` (14 passing commands; 525-test full suite). The narrow read-only runtime-probe adapter is implemented at `f0f0511` and has separately sealed fake-only evidence at `artifacts/exp-009/run-20260804T145434Z/`, generated from `a8373ad` and published at `1c995f7` (8 passing commands; 537-test full suite). The immediate Core gate is one fresh read-only ENV-001 health/load/exact-identity preflight through that adapter. No external host deployment, candidate route, full-traffic actuation, or automatic tuning is authorized.

Options considered:



- **Run a fresh read-only ENV-001 preflight through the sealed adapter:** capture exact L2 / `target-075` health, collection-load, and FLAT/HNSW identity evidence. This performs no candidate claim, approval, search, or configuration mutation.
- **Prepare the pre-registered controlled live canary:** only after the preflight, a clean revision, qualified LKG state, and an externally supplied one-time Ed25519 grant for the exact L2 / `target-075` `ef=400 → 800` transition.
